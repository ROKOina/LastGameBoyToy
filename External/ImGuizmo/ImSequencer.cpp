...
namespace ImSequencer
{
    ...
    bool Sequencer(SequenceInterface* sequence, int* currentFrame, bool* expanded, int* selectedEntry, int* firstFrame, int sequenceOptions, bool& MovingCurrentFrame, int& delID, int& addID)
    {
        ...
        if (expanded && !*expanded)
        {
            ...
            char tmps[512];
            sprintf_s(tmps, "%d Frames / %d entries", frameCount, sequenceCount);
    ...
    bool Sequencer(std::shared_ptr<MySequenceInterface> sequence, int* currentFrame, bool* expanded, int* selectedEntry, int* firstFrame, bool& MovingCurrentFrame, int sequenceOptions, SEQUENCER_RET_TYPE& retType)
    {
        bool ret = false;
        retType = SEQUENCER_RET_TYPE::RET_NONE;
        ImGuiIO& io = ImGui::GetIO();
        int cx = (int)(io.MousePos.x);
        int cy = (int)(io.MousePos.y);
        static float framePixelWidth = 10.f;
        ...
        else
        {
            ...
            */
            ...
            if (sequenceOptions & SEQUENCER_ADD)
            {
                if (SequencerAddDelButton(draw_list, ImVec2(canvas_pos.x + legendWidth - ItemHeight, canvas_pos.y + 2), true) && io.MouseReleased[0])
                {
                    sequence->Add(sequence->GetItemCount());
                    ret = true;
                    retType = SEQUENCER_RET_TYPE::RET_ADD;
                    *selectedEntry = sequence->GetItemCount() - 1;
                }
            }
            ...
            for (int i = 0; i < sequenceCount; i++)
            {
                int* start, * end;
                unsigned int color;
                sequence->Get(i, &start, &end, &color);
                size_t localCustomHeight = sequence->GetCustomHeight(i);
            ...
            if (/*backgroundRect.Contains(io.MousePos) && */movingEntry >= 0)
            {
                ...
                {
                    int* start, * end;
                    sequence->Get(movingEntry, &start, &end, NULL);
                    if (selectedEntry)
                        *selectedEntry = movingEntry;
                    int& l = *start;
                    int& r = *end;
                ...
                if (!io.MouseDown[0])
                {
                    ...
                    if (!diffFrame && movingPart && selectedEntry)
                    {
                        *selectedEntry = movingEntry;
                        ret = true;
                        retType = SEQUENCER_RET_TYPE::RET_EDIT;
                    }
        ...
        if (delEntry != -1)
        {
            ...
            ret = true;
            retType = SEQUENCER_RET_TYPE::RET_DELETE;
        }
        ...
        if (dupEntry != -1)
        {
            sequence->Duplicate(dupEntry);
            ret = true;
            retType = SEQUENCER_RET_TYPE::RET_DUPLICATE;
        }
        return ret;
    }
}