...
//マテリアル読み込み
void FbxModelResource::LoadMaterial(ID3D11Device* device, const char* filename)
{
    // ディレクトリパス取得
    char drive[32], dir[256], dirname[256];
    ::_splitpath_s(filename, drive, sizeof(drive), dir, sizeof(dir), nullptr, 0, nullptr, 0);
...
//アニメーション読み込み
void FbxModelResource::LoadAnimation(ID3D11Device* device, const char* filename)
{
    // ディレクトリパス取得
    char drive[32], dir[256], dirname[256];
    ::_splitpath_s(filename, drive, sizeof(drive), dir, sizeof(dir), nullptr, 0, nullptr, 0);
...
//ノード読み込み
void FbxModelResource::LoadNode(ID3D11Device* device, const char* filename)
{
    // ディレクトリパス取得
    char drive[32], dir[256], dirname[256];
    ::_splitpath_s(filename, drive, sizeof(drive), dir, sizeof(dir), nullptr, 0, nullptr, 0);
...
//マテリアルをエクスポート
void FbxModelResource::ExportMaterial(const char* filename)
{
    MaterialSerialize(filename);
}

//アニメーションエクスポート
void FbxModelResource::ExportAnimation(const char* filename)
{
    AnimationSerialize(filename);
}

//ノードエクスポート
void FbxModelResource::ExportNode(const char* filename)
{
    NodeSerialize(filename);
}
...
void FbxModelResource::LoadMesh(ID3D11Device* device, FbxNode* fbxNode, FbxMesh* fbxMesh)
{
    ...
    const std::string& fbxNodePath = GetNodePath(fbxNode);
...
void FbxModelResource::LoadMaterial(ID3D11Device* device, const char* dirname, FbxSurfaceMaterial* fbxSurfaceMaterial)
{
    ...
    if (fbxDiffuseProperty.IsValid())
    {
        ...
        if (fbxTextureCount > 0)
        {
            ...
            {
                // 見つかったのでそのまま設定する
                material.textureFilename[0] = relativeFileName;
            }
            else
            {
                ...
                material.textureFilename[0] = filename;
            }
        }
    }
