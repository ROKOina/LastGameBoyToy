	...
	:hWnd_(hWnd)
{
    ...
    // サンプラーステート
    {
        ...
        {
            desc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
            hr = device_->CreateSamplerState(&desc, samplerStates[SAMPLEMODE::BORDER_POINT].GetAddressOf());
            _ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
        }
        ...
        {
            desc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
            hr = device_->CreateSamplerState(&desc, samplerStates[SAMPLEMODE::BORDER_LINEAR].GetAddressOf());
            _ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
        }
        ...
        {
            desc.Filter = D3D11_FILTER_ANISOTROPIC;
            hr = device_->CreateSamplerState(&desc, samplerStates[SAMPLEMODE::BORDER_ANISOTROPIC].GetAddressOf());
            _ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
        }
...
void Graphics::SetSamplerState()
{
	...
	immediateContext_->PSSetSamplers(3, 1, samplerStates[SAMPLEMODE::BORDER_POINT].GetAddressOf());
	immediateContext_->PSSetSamplers(4, 1, samplerStates[SAMPLEMODE::BORDER_LINEAR].GetAddressOf());
	immediateContext_->PSSetSamplers(5, 1, samplerStates[SAMPLEMODE::BORDER_ANISOTROPIC].GetAddressOf());
}