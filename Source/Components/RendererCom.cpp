...
void RendererCom::EditConstantBuffer()
{
  //ImGui::Separator();
  //ImGui::Text("ConstantBuffer");

  //ModelShader* shader = Graphics::Instance().GetModelShader(shaderMode);
  //switch (shaderMode) {
  //case SHADERMODE::AREA_EFFECT_CIRCLE:
  //  shader->GetConstantBuffer<ModelShader::EffectConstants>()->data->EditGui();
