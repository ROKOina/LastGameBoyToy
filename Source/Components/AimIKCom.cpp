...
void AimIKCom::AimIK()
{
    ...
    else if (enemycopyname)
    {
        ...
        //内積を算出
        dot = DirectX::XMVectorGetX(DirectX::XMVector3Dot(DirectX::XMVector3Normalize(DirectX::XMLoadFloat3(&enemyfront)), DirectX::XMVector3Normalize(DirectX::XMVectorSubtract(DirectX::XMLoadFloat3(&GameObjectManager::Instance().Find("player")->transform_->GetWorldPosition()), DirectX::XMLoadFloat3(&GetGameObject()->transform_->GetWorldPosition())))));
        playerpos = { GameObjectManager::Instance().Find("player")->transform_->GetWorldPosition().x,GameObjectManager::Instance().Find("player")->transform_->GetWorldPosition().y - 2.5f,GameObjectManager::Instance().Find("player")->transform_->GetWorldPosition().z };
