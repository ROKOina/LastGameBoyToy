...
#define scaleKeyCount 5
	...
	struct ScaleLifeTime
	{
		...
		DirectX::XMFLOAT2 value{ 1,1 };
		DirectX::XMFLOAT2 curvePower{ 1,1 };	//valueに行くまでのカーブ
...
#define colorKeyCount 5
	...
	struct ParticleConstantsA
	{
		...
		ScaleLifeTimeA scaleLifeTime[scaleKeyCount];	//並び替えてkey順にする
		ScaleLifeTimeA scaleLifeTimeRand[scaleKeyCount];
	...
	struct ParticleConstants
	{
		...
		DirectX::XMFLOAT4 startSize{ 0.5f, 0.5f, 0, 0};	//xy:サイズ　zw:乱数用サイズ
		float startSizeRandom = 0;	//1:乱数使用 2:乱数オフ

		DirectX::XMFLOAT3 pcDummy{ 0, 0, 0};
	...
	//シリアルする情報
	struct SaveParticleDataA
	{
		std::string particleTexture;
		ParticleConstantsA particleData;
...
private:
	...
	SaveParticleDataA particleDataA_;
