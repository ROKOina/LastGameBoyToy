...
float4 main(float4 position : SV_POSITION, float2 texcoord : TEXCOORD) : SV_TARGET
{
    ...
    float4 sampled_color = hdr_color_buffer_texture.Sample(sampler_states[BORDER_POINT], texcoord) * weight[0];
    for (int i = 1; i < 3; i++)
    {
        sampled_color += hdr_color_buffer_texture.Sample(sampler_states[BORDER_POINT], texcoord + float2(0.0, offset[i] / height)) * weight[i];
        sampled_color += hdr_color_buffer_texture.Sample(sampler_states[BORDER_POINT], texcoord - float2(0.0, offset[i] / height)) * weight[i];
    }
